 ...
 */
type ChangeEventCallback = (change: m.ChangeStreamDocument<m.BSON.Document>) => void;
...
export function watchEvent<TSchema extends m.Document = m.Document>(
    ...
    if (!watchers) {
        ...
        changeStream.on("change", (change) => {
            ...
            const collectionName = (change as unknown as { ns?: { coll?: string } }).ns?.coll;
    ...
    return () => {
        ...
        if (watchers) {
            ...
            if(namespace) {
                watchers.namespaceListener[collection.collectionName] = namespace.filter((l) => l !== callback);
            }
