...
import type { MigrationRule, MigrationState } from "../types.ts";
import type {
  SimulationApplier,
...
export class IntegrityValidator {
  ...
  validateMigrationState(
    ...
    for (const operation of migrationState.operations) {
      ...
      operationTypes[operation.type] = (operationTypes[operation.type] || 0) +
        1;
    ...
    if (
      ...
      migrationState.operations.length > this.options.maxOperations
    ) {
      errors.push(
        `Migration exceeds maximum operations limit: ${migrationState.operations.length} > ${this.options.maxOperations}`,
      );
    }
    ...
    return {
      ...
      metadata: {
        ...
        performance: {
          ...
          simulationDuration: simulationDuration > 0
            ? simulationDuration
            : undefined,
        },
      },
  ...
  private validateCreateCollectionOperation(
    ...
    if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(operation.collectionName)) {
      warnings.push(
        `Collection name "${operation.collectionName}" should start with letter and contain only letters, numbers, and underscores`,
      );
    }
  }
  ...
  private validateTransformCollectionOperation(
    ...
    if (!("down" in operation) || typeof operation.down !== "function") {
      ...
      warnings.push(
        "transform_collection reversibility not validated (validateReversibility disabled)",
      );
    }
  }
  ...
  private runSimulation(
    ...
    try {
      ...
      for (const operation of migrationState.operations) {
        currentState = this.simulationApplier.applyOperation(
          currentState,
          operation,
        );
      }
      ...
      if (this.options.validateReversibility) {
        try {
          ...
          for (let i = migrationState.operations.length - 1; i >= 0; i--) {
            ...
            reverseState = this.simulationApplier.applyReverseOperation(
              reverseState,
              operation,
            );
          }
