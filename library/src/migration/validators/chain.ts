...
export class ChainValidator {
  ...
  private validateBasicStructure(
    ...
    for (const migration of migrations) {
      if (!migration.id || typeof migration.id !== "string") {
        errors.push(
          `Migration missing or invalid ID: ${JSON.stringify(migration.id)}`,
        );
      }
      ...
      if (this.options.strictIdFormat && migration.id) {
        if (!/^[a-zA-Z0-9_@-]+$/.test(migration.id)) {
          errors.push(
            `Migration ID "${migration.id}" contains invalid characters. Use only letters, numbers, underscores, hyphens, and @`,
          );
        }
      }
   ...
   * @private
   */
  private validateParentReferences(
    migrations: readonly MigrationDefinition[],
    errors: string[],
  ): void {
    ...
    for (const migration of migrations) {
      if (migration.parent && !idSet.has(migration.parent.id)) {
        errors.push(
          `Migration "${migration.id}" references non-existent parent "${migration.parent.id}"`,
        );
      }
    }
  }
  ...
  private analyzeChainStructure(
    ...
    for (const migration of migrations) {
      ...
      if (
        !childrenMap.has(migration.id) ||
        childrenMap.get(migration.id)!.size === 0
      ) {
  ...
  private validateRootsAndLeaves(
    ...
    if (roots.length === 0) {
      errors.push(
        "Migration chain must have at least one root migration (with parent: null)",
      );
    ...
    if (leaves.length === 0) {
      warnings.push(
        "No leaf migrations found. This may indicate a complex branching structure",
      );
  ...
  private validateDepth(depth: number, errors: string[]): void {
    if (this.options.maxDepth && depth > this.options.maxDepth) {
      errors.push(
        `Migration chain depth ${depth} exceeds maximum allowed depth ${this.options.maxDepth}`,
      );
    }
  }
  ...
  private validateOperationIntegrity(
    ...
    for (const migration of migrations) {
      ...
      if (!migration.migrate || typeof migration.migrate !== "function") {
        errors.push(
          `Migration "${migration.id}" missing or invalid migrate function`,
        );
        continue;
      }
      ...
      if (!migration.schemas || typeof migration.schemas !== "object") {
        warnings.push(
          `Migration "${migration.id}" missing or invalid schemas definition`,
        );
      }
