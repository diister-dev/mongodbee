...
export type MigrationValidator = {
  ...
  validateOperation: (operation: MigrationRule, context: MigrationExecutionContext) => Promise<ValidationResult>;
  
  /** Validates the final state after migration */
  validateState: (state: MigrationState, context: MigrationExecutionContext) => Promise<ValidationResult>;
};
...
export type MigrationRunner = {
  ...
  executeMigration: (definition: MigrationDefinition) => Promise<MigrationExecutionResult>;
  
  /** Execute multiple migrations in sequence */
  executeMigrations: (definitions: MigrationDefinition[]) => Promise<MigrationExecutionResult[]>;
  
  /** Rollback a migration */
  rollbackMigration: (definition: MigrationDefinition) => Promise<MigrationExecutionResult>;
 ...
 */
export function createConsoleLogger(level: 'debug' | 'info' | 'warn' | 'error' = 'info'): MigrationLogger {
...
async function executeOperationWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      logger?.debug(`Operation ${operation.type} completed successfully`, { attempt });
      return { success: true, attempts: attempt };
      
    } catch (error) {
      lastError = error instanceof Error ? error : new Error(String(error));
      logger?.warn(`Operation ${operation.type} failed on attempt ${attempt}`, { error: lastError.message });
...
export function createMigrationRunner(
  ...
  return {
    ...
    async executeMigration(definition: MigrationDefinition): Promise<MigrationExecutionResult> {
        ...
        if (!mergedConfig.dryRun) {
          ...
          for (let i = 0; i < operations.length; i++) {
            ...
            if (context.validator) {
              const opValidation = await context.validator.validateOperation(operation, context);
              if (!opValidation.success) {
                const opErrors = opValidation.errors.map(err => `Operation ${i + 1} (${operation.type}): ${err}`);
                errors.push(...opErrors);
                
                if (!mergedConfig.continueOnErrors) {
                  await context.onOperation?.(operation, 'error', context, new Error(opErrors.join(', ')));
                  break;
                }
              }
            ...
            const result = await executeOperationWithRetry(operation, context.applier, mergedConfig, logger);
            
            if (result.success) {
              appliedOperations++;
              logger.debug(`Operation ${i + 1}/${operations.length} completed`, { type: operation.type });
              await context.onOperation?.(operation, 'after', context);
            } else {
              const error = result.error || new Error('Unknown operation error');
              errors.push(`Operation ${i + 1} (${operation.type}) failed after ${result.attempts} attempts: ${error.message}`);
          ...
          logger.info('Dry-run mode: simulating operations without applying changes');
          appliedOperations = operations.length; // In dry-run, we "simulate" all operations
        }
        
        // Post-execution validation
        let finalValidation: ValidationResult = { success: true, errors: [], warnings: [] };
        if (mergedConfig.validateAfterExecution && context.validator) {
          logger.info('Validating migration after execution');
          finalValidation = await context.validator.validateState(state, context);
    ...
    async executeMigrations(definitions: MigrationDefinition[]): Promise<MigrationExecutionResult[]> {
      const results: MigrationExecutionResult[] = [];
      
      logger.info(`Starting batch migration of ${definitions.length} migrations`);
      
      for (let i = 0; i < definitions.length; i++) {
        const definition = definitions[i];
        logger.info(`Executing migration ${i + 1}/${definitions.length}: ${definition.name}`);
        ...
        if (!result.success && !mergedConfig.continueOnErrors) {
          logger.error(`Stopping batch execution due to failure in migration: ${definition.name}`);
          break;
        }
      }
      
      const successful = results.filter(r => r.success).length;
      logger.info(`Batch migration completed: ${successful}/${definitions.length} successful`);
      
      return results;
    },
    
    async rollbackMigration(definition: MigrationDefinition): Promise<MigrationExecutionResult> {
      ...
      const rollbackApplier: MigrationApplier = {
        applyOperation: (operation) => originalApplier.applyReverseOperation(operation),
        applyReverseOperation: (operation) => originalApplier.applyOperation(operation),
      };
      ...
      const rollbackRunner = createMigrationRunner(rollbackContext, mergedConfig);
      return await rollbackRunner.executeMigration(rollbackDefinition);
    },
    
    async validateMigration(definition: MigrationDefinition): Promise<ValidationResult> {
      if (!context.validator) {
        return {
          success: true,
          errors: [],
