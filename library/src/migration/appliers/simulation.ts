...
export class SimulationApplier implements SimulationMigrationApplier {
  ...
  private applyTransformMultiCollectionType(
    ...
    if(!state.multiCollections) {
      throw new Error(`No multi-collections exist to transform type ${operation.typeName}`);
    }

    const collectionElement = state.multiCollections[operation.collectionType];
    for(const [index, doc] of collectionElement.content.entries()) {
      if(doc?._type !== operation.typeName) continue;
      collectionElement.content[index] = operation.up(doc);
    }
  ...
  private reverseTransformMultiCollectionType(
    ...
    if(!state.multiCollections) {
      throw new Error(`No multi-collections exist to reverse transform type ${operation.typeName}`);
    }

    const collectionElement = state.multiCollections[operation.collectionType];
    for(const [index, doc] of collectionElement.content.entries()) {
      if(doc?._type !== operation.typeName) continue;
      collectionElement.content[index] = operation.down(doc);
    }
