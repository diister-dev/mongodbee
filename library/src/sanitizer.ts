 ...
 */
export function sanitizeDocument(obj: any, options: SanitizeOptions = { undefinedBehavior: 'remove', deep: true }): any {
    if (!options.deep && typeof obj === 'object' && obj !== null && !Array.isArray(obj)) {
        ...
        for (const [key, value] of Object.entries(obj)) {
            ...
            result[key] = options.undefinedBehavior === 'convert-to-null' && value === undefined ? null : value;
        }
        return result;
    }

    // Deep sanitization
    return options.undefinedBehavior === 'remove' ? removeUndefined(obj) : undefinedToNull(obj);
}
    ...
    function processValue(value: unknown): unknown {
        ...
        if (value === undefined) {
            switch (options.undefinedBehavior) {
                ...
                case 'error':
                    throw new Error('Undefined values are not allowed. Use removeField() to explicitly remove fields.');
                default:
                    return undefined;
            }
        }
        ...
        if (Array.isArray(value)) {
            return value.map(processValue).filter(item => item !== undefined && item !== IGNORE_FIELD);
        }
