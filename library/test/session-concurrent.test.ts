...
test("Session Concurrent: Regular collection operations inside and outside session", async () => {
  await withDatabase("Session Concurrent: Regular collection operations inside and outside session", async (db) => {
    ...
    const sessionUpdatePromise = users.withSession(async () => {
      ...
      expect(userInSession.age).toEqual(31);
      expect(userInSession.version).toEqual(2);

      const productInSession = await products.getById(productId);
      expect(productInSession.stock).toEqual(19);
      expect(productInSession.version).toEqual(2);
    ...
    expect(userOutsideSession.age).toEqual(30);
    expect(userOutsideSession.version).toEqual(1);

    const productOutsideSession = await products.getById(productId);
    expect(productOutsideSession.stock).toEqual(20);
    expect(productOutsideSession.version).toEqual(1);
    ...
    expect(finalUser.age).toEqual(31);
    expect(finalUser.version).toEqual(2);

    const finalProduct = await products.getById(productId);
    expect(finalProduct.stock).toEqual(19);
    expect(finalProduct.price).toEqual(95); // This update was made outside the transaction
    expect(finalProduct.version).toEqual(2);
  });
});

test("Session Concurrent: MultiCollection operations inside and outside session", async () => {
  await withDatabase("Session Concurrent: MultiCollection operations inside and outside session", async (db) => {
    ...
    const sessionUpdatePromise = store.withSession(async () => {
      ...
      await store.updateById("category", categoryId, {
      ...
      expect(productInSession).not.toBeNull();
      expect(productInSession!.stock).toEqual(19);
      expect(productInSession!.version).toEqual(2);
      ...
      expect(categoryInSession).not.toBeNull();
      expect(categoryInSession!.productCount).toEqual(2);
    ...
    expect(productOutsideSession).not.toBeNull();
    expect(productOutsideSession!.stock).toEqual(20);
    expect(productOutsideSession!.version).toEqual(1);
    ...
    expect(categoryOutsideSession).not.toBeNull();
    expect(categoryOutsideSession!.productCount).toEqual(1);
    ...
    expect(finalCategory!.productCount).toEqual(2);
  });
});

test("Session Concurrent: Mixed collection types with concurrent operations", async () => {
  await withDatabase("Session Concurrent: Mixed collection types with concurrent operations", async (db) => {
    ...
    const sessionUpdatePromise = users.withSession(async () => {
      ...
      const userInSession = await users.getById(userId);
      expect(userInSession).not.toBeNull();
      expect(userInSession.age).toEqual(36);

      const productInSession = await store.findOne("product", {
        _id: productId,
      });
      expect(productInSession).not.toBeNull();
      expect(productInSession!.stock).toEqual(49);
    ...
    const userOutsideSession = await users.getById(userId);
    expect(userOutsideSession).not.toBeNull();
    expect(userOutsideSession.age).toEqual(35);
    expect(userOutsideSession.version).toEqual(1);
    ...
    expect(productOutsideSession).not.toBeNull();
    expect(productOutsideSession!.stock).toEqual(50);
    expect(productOutsideSession!.version).toEqual(1);
    ...
    expect(finalUser.age).toEqual(36);
    expect(finalUser.version).toEqual(2);
    expect(finalUser.email).toEqual("jane.smith@example.com");

    const finalProduct = await store.findOne("product", { _id: productId });
    expect(finalProduct).not.toBeNull();
    expect(finalProduct!.stock).toEqual(49);
    expect(finalProduct!.version).toEqual(2);
    expect(finalProduct!.price).toEqual(549);
  });
});

test("Session Concurrent: Rollback shouldn't affect outside operations", async () => {
  await withDatabase("Session Concurrent: Rollback shouldn't affect outside operations", async (db) => {
    // Create regular collection and multi-collection
    const users = await collection(db, "users", userSchema);
    const store = await multiCollection(db, "store", storeModel);
    ...
    if (error instanceof Error) {
      expect(error.message).toEqual("Intentional error to trigger rollback");
    } else {
    ...
    const finalUser = await users.getById(userId);
    expect(finalUser).not.toBeNull();
    expect(finalUser.age).toEqual(40); // Session changes rolled back
    expect(finalUser.version).toEqual(1); // Session changes rolled back
    expect(finalUser.email).toEqual("alex.brown@example.com"); // Outside changes preserved

    const finalProduct = await store.findOne("product", { _id: productId });
    expect(finalProduct).not.toBeNull();
    expect(finalProduct!.stock).toEqual(10); // Session changes rolled back
    expect(finalProduct!.version).toEqual(1); // Session changes rolled back
    expect(finalProduct!.price).toEqual(1100); // Outside changes preserved
  });
});
