...
test("chain-based and timestamp-based skip diverge on out-of-order IDs", () => {
...
test("chain-based: instance@m1 should receive m2 (older instance, newer migration)", () => {
  const m1 = makeMigration("001", null);
  const m2 = makeMigration("002", m1);
  assertEquals(shouldInstanceReceiveMigrationByChain(m1, m2), true);
});

test("chain-based: instance@m2 should NOT receive m1 (newer instance, older migration)", () => {
  const m1 = makeMigration("001", null);
  const m2 = makeMigration("002", m1);
  assertEquals(shouldInstanceReceiveMigrationByChain(m2, m1), false);
});

test("chain-based: instance receives the migration it was created at", () => {
  const m1 = makeMigration("001", null);
