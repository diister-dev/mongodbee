...
test("Validation Attack 1: Schema change without transformation should fail", async () => {
  ...
  const attackMigration = migrationDefinition(
    "2025_01_02_ATTACK1",
    "add_age_no_transform",
    {
...
test("Valid transformation with correct values should pass", async () => {
  ...
  const validMigration = migrationDefinition(
    "2025_01_02_VALID",
    "add_age_correct",
    {
      parent: rootMigration,
  ...
  assertEquals(
    result.success,
    true,
    "Valid migration should pass without errors",
  );
});
