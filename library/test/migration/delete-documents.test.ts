...
function parentMigration(): MigrationDefinition {
  return migrationDefinition("001", "seed-everything", {
    ...
    migrate: (b) => {
      ...
      b.createCollection("posts").seed([
        { _id: "post:1", authorId: "user:b", title: "hello" },
      ]).end();
      b.createMultiModelInstance("shop:main", "shop")
        ...
        .type("order").seed([{ _id: "order:1", itemId: "item:1", qty: 1 }])
        .end()
        .end();
      b.createMultiModelInstance("shop:other", "shop")
        .type("item").seed([{ _id: "item:3", sku: "C", stock: 0 }]).end()
        .end();
      b.createScopedMultiCollection("+expo").type("ticket")
        .seed("exposition:x", [
...
async function memoryStateAfter(
  ...
  const parentOps =
    parent.migrate(migrationBuilder({ schemas: parent.schemas })).operations;
...
Deno.test("deleteWhere on a collection: operators, irreversibility, memory and mongo agree", async () => {
  const parent = parentMigration();
  const child = childMigration(parent, (b) => {
    b.collection("users").deleteWhere({ status: "banned", age: { $lt: 18 } })
      .end();
    return b.compile();
  });
  const { state, childOps } = await memoryStateAfter(parent, child);
  assertEquals(state.collections.users.content.map((u) => String(u._id)), [
    "user:a",
    "user:c",
  ]);
  ...
  await withDatabase("delete-collection-documents", async (db) => {
    const parentOps =
      parent.migrate(migrationBuilder({ schemas: parent.schemas })).operations;
    await createMongodbApplier(db, parent, { currentMigrationId: parent.id })
      .applyMigration(parentOps, "up");
    await createMongodbApplier(db, child, { currentMigrationId: child.id })
      .applyMigration(childOps, "up");
    const ids =
      (await db.collection("users").find({}).sort({ _id: 1 }).toArray()).map((
        u,
      ) => String(u._id));
...
Deno.test("deleteWhere on a scoped type honours the scope filter and never leaves its type", async () => {
  const parent = parentMigration();
  const child = childMigration(parent, (b) => {
    b.scopedMultiCollection("+expo").type("ticket").deleteWhere(
      { used: true },
      { scopeFilter: ["exposition:x"] },
    ).end().end();
    return b.compile();
  });
  const { state, childOps } = await memoryStateAfter(parent, child);
  const remaining = state.scopedMultiCollections["+expo"].content.map((d) =>
    String(d._id)
  ).sort();
  ...
  await withDatabase("delete-scoped-documents", async (db) => {
    ...
    await createMongodbApplier(db, child, { currentMigrationId: child.id })
      .applyMigration(childOps, "up");
    const ids =
      (await db.collection("+expo").find({ _type: "ticket" }).sort({ _id: 1 })
        .toArray()).map((d) => String(d._id));
    assertEquals(ids, ["ticket:2", "ticket:3"]);
  });
});

Deno.test("deleteWhere on one multi-model instance and on every instance of a model", async () => {
  const parent = parentMigration();
  const one = childMigration(parent, (b) => {
    b.multiModelInstance("shop:main", "shop").type("item").deleteWhere({
      stock: 0,
    })
      .end().end();
    return b.compile();
  ...
  const all = childMigration(parent, (b) => {
    b.multiModelInstances("shop").type("item").deleteWhere({ stock: 0 }).end()
      .end();
    return b.compile();
  });
  ...
  await withDatabase("delete-multimodel-documents", async (db) => {
    ...
    await createMongodbApplier(db, parent, { currentMigrationId: parent.id })
      .applyMigration(parentOps, "up");
    await createMongodbApplier(db, all, { currentMigrationId: all.id })
      .applyMigration(childOps, "up");
    assertEquals(
...
Deno.test("simulation warns when a deletion matches nothing and when it leaves dangling references", async () => {
  const parent = parentMigration();
  ...
  assert(
    dangling.warnings.some((w) =>
      w.includes("still referenced") && w.includes("authorId")
    ),
  ...
  return migrationDefinition("001", "seed-isolation", {
    ...
    migrate: (b) => {
      b.createMultiCollection("+tenants")
        .type("role").seed([{ _id: "role:1", name: "x", flagged: true }]).end()
        .type("member").seed([{ _id: "member:1", name: "x", flagged: true }])
        .end()
        .end();
...
async function isolationRun(
  ...
  const memoryIds = {
    tenants: state.multiCollections["+tenants"].content.map((d) =>
      String(d._id)
    ).sort(),
    expo: state.scopedMultiCollections["+expo"].content.map((d) =>
      String(d._id)
    ).sort(),
  };
  ...
  await withDatabase("delete-isolation", async (db) => {
    ...
    await createMongodbApplier(db, child, { currentMigrationId: child.id })
      .applyMigration(childOps, "up");
    mongoIds = {
      tenants: (await db.collection("+tenants").find({}).toArray()).map((d) =>
        String(d._id)
      ).sort(),
      expo: (await db.collection("+expo").find({}).toArray()).map((d) =>
        String(d._id)
      ).sort(),
    };
  });
...
Deno.test("deleteWhere never crosses a type boundary, in a multi-collection or a scoped one", async () => {
  const parent = isolationParent();
  const child = migrationDefinition("002", "delete-flagged", {
    parent,
    schemas: ISOLATION,
    migrate: (b) => {
      b.multiCollection("+tenants").type("role").deleteWhere({ flagged: true })
        .end().end();
      b.scopedMultiCollection("+expo").type("ticket").deleteWhere({
        flagged: true,
      }).end().end();
...
Deno.test("deleteWhere with a scope filter touches only the listed scopes, and reserved keys are refused", async () => {
  const parent = isolationParent();
  assertThrows(
    () =>
      migrationDefinition("002", "delete-widening", {
        ...
        schemas: ISOLATION,
        migrate: (b) => {
          b.scopedMultiCollection("+expo").type("ticket")
            .deleteWhere({ flagged: true, _scope: "exposition:c" }, {
  ...
  assertThrows(
    () =>
      migrationDefinition("002", "delete-retyping", {
        ...
        schemas: ISOLATION,
        migrate: (b) => {
          b.multiCollection("+tenants").type("role").deleteWhere({
            _type: "member",
          }).end().end();
          return b.compile();
        },
...
Deno.test("simulation exercises a scoped deletion and reports the unexercised one", async () => {
  const parent = isolationParent();
  const validator = createSimulationValidator({ powerLevel: "quick" });
