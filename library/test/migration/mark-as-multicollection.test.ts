...
import { MULTI_COLLECTION_INFO_TYPE, MULTI_COLLECTION_MIGRATIONS_TYPE } from "../../src/migration/multicollection-registry.ts";
...
Deno.test("markAsMultiCollection - creates correct operation rule", () => {
  ...
  assertEquals((state.operations[0] as MarkAsMultiCollectionRule).collectionName, "catalog_main");
  assertEquals((state.operations[0] as MarkAsMultiCollectionRule).collectionType, "catalog");
});
...
Deno.test("markAsMultiCollection - creates metadata documents", async (t) => {
  await withDatabase(t.name, async (db) => {
    ...
    const infoDoc = await collection.findOne({ _type: MULTI_COLLECTION_INFO_TYPE });
    assertExists(infoDoc, "Info document should be created");
    assertEquals(infoDoc.collectionType, "catalog");
    
    const migrationsDoc = await collection.findOne({ _type: MULTI_COLLECTION_MIGRATIONS_TYPE });
    assertExists(migrationsDoc, "Migrations document should be created");
    
    // Original documents should still exist
    const count = await collection.countDocuments({ _type: { $exists: false } });
...
Deno.test("markAsMultiCollection - is idempotent", async (t) => {
  await withDatabase(t.name, async (db) => {
    ...
    await collection.insertMany([
      { _type: MULTI_COLLECTION_INFO_TYPE, collectionType: "catalog", migrationId: "mig_old" },
...
Deno.test("markAsMultiCollection - reverses by removing metadata", async (t) => {
  await withDatabase(t.name, async (db) => {
    ...
    await collection.insertMany([
      { _type: MULTI_COLLECTION_INFO_TYPE, collectionType: "catalog", migrationId: "mig_test" },
      { _type: MULTI_COLLECTION_MIGRATIONS_TYPE, migrations: [] },
...
Deno.test("markAsMultiCollection - full apply-reverse cycle", async (t) => {
  await withDatabase(t.name, async (db) => {
    ...
    let metadataCount = await collection.countDocuments({ _type: { $exists: true } });
    assertEquals(metadataCount, 2, "Should have 2 metadata docs after apply");
    
    // Reverse: remove marking
    await applier.applyReverseOperation(operation);
    
    metadataCount = await collection.countDocuments({ _type: { $exists: true } });
    assertEquals(metadataCount, 0, "Should have no metadata docs after reverse");
