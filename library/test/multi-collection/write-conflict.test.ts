  ...
  fn: async (t) => {
    await withDatabase(t.name, async (db) => {
      ...
      try {
        ...
        console.log(
          "Write conflict occurred (expected):",
...
Deno.test({
  ...
  fn: async (t) => {
    await withDatabase(t.name, async (db) => {
      ...
      const sequentialResult = await store.findOne("counter", {
        _id: counterId1,
      });
      ...
      try {
        ...
        const concurrentResult = await store.findOne("counter", {
          _id: counterId2,
        });
        assert(concurrentResult !== null);
        console.log(
          `Concurrent result: ${concurrentResult.value} (may differ from 5)`,
        );
      } catch (error) {
        console.log(
          "Concurrent updates failed with:",
          error instanceof Error ? error.message : error,
        );
      }
    });
    ...
    await withDatabase(t.name, async (db) => {
      ...
      try {
        ...
        if (error instanceof Error) {
          ...
          if (isWriteConflict) {
            console.log(
              "✓ Write conflict detected in concurrent transactions (expected)",
            );
          }
        }
      }
...
Deno.test("Write Conflict: Stress test with many concurrent operations", async (t) => {
  await withDatabase(t.name, async (db) => {
    ...
    console.log(
      `Stress test results: ${successCount} succeeded, ${failureCount} failed`,
    );
