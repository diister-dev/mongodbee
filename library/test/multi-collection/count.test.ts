...
Deno.test("MultiCollection: countDocuments functionality", async (t) => {
    await withDatabase(t.name, async (db) => {
        ...
        await catalog.insertOne("product", { name: "Laptop", price: 999, category: "Electronics" });
        await catalog.insertOne("product", { name: "Phone", price: 499, category: "Electronics" });
        await catalog.insertOne("product", { name: "Novel", price: 15, category: "Books" });
        ...
        const electronicsCount = await catalog.countDocuments("product", { category: "Electronics" });
        assertEquals(electronicsCount, 2);

        const booksCount = await catalog.countDocuments("product", { category: "Books" });
        assertEquals(booksCount, 1);

        // Test counting with price filter (Laptop: 999 >= 500, Phone: 499 < 500, Novel: 15 < 500)
        const expensiveCount = await catalog.countDocuments("product", { price: { $gte: 500 } });
        ...
        const electronicsCategory = await catalog.countDocuments("category", { name: "Electronics" });
        assertEquals(electronicsCategory, 1);
        
        const booksCategory = await catalog.countDocuments("category", { name: "Books" });
