...
test("auto-index: { _scope: 1, _type: 1 } is created at init", async () => {
  await withDatabase("smc-idx-base", async (db) => {
    ...
    const found = idx.find(
      (i) => i.key && i.key._scope === 1 && i.key._type === 1,
    );
    assert(
      found,
...
test("scoped uniqueness: same email in two scopes is allowed", async () => {
...
test("scoped uniqueness: duplicate email in same scope+type fails", async () => {
...
test("global uniqueness: same field across scopes is rejected", async () => {
...
test("uniqueness is scoped per-type: same field name across two types does not collide", async () => {
...
test("scoped-unique index has compound shape { _scope:1, _type:1, field:1 }", async () => {
...
test("global-unique index has compound shape { _type:1, field:1 } (no _scope)", async () => {
...
test("double init: adding a sibling type with the same unique field keeps both indexes", async () => {
...
test("legacy multiCollection indexes are cleaned up when re-opened as scoped", async () => {
  await withDatabase("smc-idx-legacy-cleanup", async (db) => {
    ...
    assert(
      ...
      `legacy user_email index should be dropped, got ${JSON.stringify(after)}`,
    );
...
test("N3: base index is {_scope:1,_type:1,_id:1} and stable across re-inits", async () => {
  await withDatabase("smc-idx-base-idn3", async (db) => {
    ...
    assert(
      base,
      `expected base index _scope_1__type_1__id_1, got ${JSON.stringify(
        first.map((i) => i.name),
      )}`,
    );
    ...
    assertEquals(
      names2,
      names1,
      `re-init changed the index set: ${JSON.stringify(names1)} -> ${JSON.stringify(
        names2,
      )}`,
    );
  });
...
test("N3 migration: old {_scope:1,_type:1} base index is replaced by {_scope:1,_type:1,_id:1}", async () => {
  await withDatabase("smc-idx-base-migration", async (db) => {
    ...
    await db
      .collection("catalog")
      .createIndex({ _scope: 1, _type: 1 }, { name: "_scope_1__type_1" });
    ...
    assertEquals(
      afterNames2,
      afterNames1,
      `second init oscillated: ${JSON.stringify(afterNames1)} -> ${JSON.stringify(
        afterNames2,
      )}`,
    );
  });
...
test("N4: a non-unique {_type:1} index (_type_1) is always created", async () => {
...
test("N5: plain collection() with { global: true } index metadata initializes without throwing", async () => {
  await withDatabase("smc-idx-global-strip", async (db) => {
    ...
    await assertRejects(() => users.insertOne({ slug: "a", name: "n2" }));
  });
});

// [N5] Same guarantee on the `multiCollection` applier path.
test("N5: multiCollection with { global: true } index metadata initializes without throwing", async () => {
  await withDatabase("smc-idx-global-strip-mc", async (db) => {
    ...
    assert(
      ...
      `expected unique item_slug index, got ${JSON.stringify(
        idx.map((i) => i.name),
      )}`,
    );
  });
...
test("N5 (unit): applyCollectionIndexes strips `global` before createIndex", async () => {
...
test("N5 (unit): applyMultiCollectionIndexes strips `global` before createIndex", async () => {
